...
#文件下载器
def Down_load(file_url , file_name , now_photo_count , all_photo_count):
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36"}

    #开始下载图片
    with closing(get(url=file_url , headers=headers ,stream = True)) as response:
        chunk_size = 1024 #单次下载最大值
        ...
        with open(file_name,"wb") as file:
            for data in response.iter_content(chunk_size=chunk_size):
                ...
                print("\r %s：[%s%s] %d%% %d/%d" % (file_name, done_block * '█', ' ' * (50 - 1 - done_block), now_jd, now_photo_count,all_photo_count), end=" ")

        # 下载完图片后获取图片扩展名，并为其增加扩展名
    file_type = guess(file_name)
    rename(file_name, file_name + '.' + file_type.extension)
