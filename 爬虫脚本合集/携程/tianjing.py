...
def get_detail_url_list():
    ...
    #try:
    url = "https://vacations.ctrip.com/list/grouptravel/d-hubei-100067.html?cityname=成都&salecity=28&startcity=28&filter={0}&p={1}"
    ...
    for i in range(0,len(filter_list)):
        ...
        p = 1
        while 1:
            res = requests.get(url=url.format(filter_list[i],str(p)),headers=headers,proxies={"http": "http://{}".format(proxy)})

            print("cur_url:",url.format(filter_list[i],str(p)))
            pat = '"products":(.*?),"crumbs":'
            ...
            parsing_detail_url_list(detail_url_list,products_id_list,filter_list[i])
            time.sleep(2)

    # except Exception as e:
    #     print(e)

def parsing_detail_url_list(detail_url_list,products_id_list,filter):
    ...
    for i in range(0,len(products_id_list)):
        ...
        if len(price) == 0:
            print("price is None")
            price = 868
            write_to_world(filter, title[0], price, point_list)
        else:
            print("当前价格为", price[0], "元/人")
            write_to_world(filter,title[0],price[0],point_list)
        time.sleep(2)
    #except Exception as e:
        #print(e)

def write_to_world(filter,title,price,point_list):
    route = """"""
