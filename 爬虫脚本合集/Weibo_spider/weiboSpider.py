...
from weiboFollow import Follow
...
class Weibo(object):
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            if type == 'img' or type == 'video':
                return file_dir
            if type == 'follow':
                file_path = file_dir + os.sep + self.user_config['user_id']+'_follow'+ '.csv'
                return file_path
            if type == 'user_info':
                file_path = os.path.split(os.path.realpath(__file__))[0] + os.sep + 'weibo' + os.sep
                return file_path
            if type == '_follow.html':
                file_path = file_dir + os.sep + self.user_config['user_id']+'_follow'+ '.html'
                return file_path
    ...
    def write_user_csv(self):
        """将用户的信息写入csv文件"""
        fieldnames = ["ID", "nickname", "gender", "location","birthday","description","verified_reason","talent","education","work","weibo_num","following","followers"]
        file_path = self.get_filepath('user_info') + 'user_info.csv'
        with open(file_path,'a',encoding='utf-8-sig',newline='') as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            if os.path.getsize(file_path) == 0:
                writer.writeheader()
            writer.writerow({'ID': self.user['id'],
                             'nickname': self.user['nickname'],
                             'gender': self.user['gender'],
                             'location': self.user['location'],
                             'birthday': self.user['birthday'],
                             'description': self.user['description'],
                             'verified_reason': self.user['verified_reason'],
                             'talent': self.user['talent'],
                             'education': self.user['education'],
                             'work': self.user['work'],
                             'weibo_num': self.user['weibo_num'],
                             'following': self.user['following'],
                             'followers': self.user['followers'],
                             })
            f.close()
            user_info_html = self.get_filepath('user_info')  + 'user_info.html'
            os.system('csvtotable -o ' + file_path + ' '+user_info_html)
            print(self.user['nickname'],"信息已保存")
    ...
    def write_csv(self, wrote_num):
        ...
        try:
            ...
            if sys.version < '3':  # python2.x
                ...
                os.system('csvtotable -o '+file_path +' '+self.get_filepath('html'))
    ...
    def write_user_follwo_csv(self):
        fieldnames = ['ID', 'nickname']
        filepath = self.get_filepath('follow')
        print("follow的filepath为：",filepath)
        with open(filepath, 'a', encoding='utf-8_sig', newline='') as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            if os.path.getsize(filepath) == 0:
                writer.writeheader()
            for user in self.follwo.follow_list:
                writer.writerow({
                    'ID': user['uri'],
                    'nickname': user['nickname']
                })
            csv_file.close()
            csv_save_path = self.get_filepath('_follow.html')
            os.system('csvtotable -o ' + filepath + ' ' + csv_save_path)
...
def main():
    try:
        ...
        wb = Weibo(config)
        wb.follwo.start() #开始抓取关注用户
        wb.start()  # 爬取微博信息
        wb.write_user_follwo_csv()  # 保存关注用户列表
    except Exception as e:
        print('Error: ', e)
        traceback.print_exc()
